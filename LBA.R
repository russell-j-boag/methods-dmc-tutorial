...
# Plot RT histograms
par(mfcol=c(2,2))
hist(dat$RT[(dat$D=='easy' & dat$C==1) & dat$I=='spd'], breaks=20, xlab='RT (s)', 
     main='Easy, Spd', xlim=c(.2,1.5))  # easy, speed condition
hist(dat$RT[(dat$D=='easy' & dat$C==1) & dat$I=='acc'], breaks=20, xlab='RT (s)', 
     main='Easy, Acc', xlim=c(.2,1.5))  # easy, accuracy condition
hist(dat$RT[(dat$D=='hard' & dat$C==1) & dat$I=='spd'], breaks=20, xlab='RT (s)', 
     main='Hard, Spd', xlim=c(.2,2.2))  # hard, speed condition
hist(dat$RT[(dat$D=='hard' & dat$C==1) & dat$I=='acc'], breaks=20, xlab='RT (s)', 
...
# Build model
model <- model.dmc(p.map     = list(A="1",B="1",t0="1",mean_v=c("D","M"),sd_v="M",st0="1"), 
                   match.map = list(M=list(left="L",right="R")),
                   factors=list(S=c("left","right"),D=c("easy","hard"),I=c("acc","spd")),
...
## Step 1 - Build model and set prior parameter values
# Build model
model <- model.dmc(p.map     = list(A="1",B="I",t0="1",mean_v=c("M"),sd_v="M",st0="1"), 
                   match.map = list(M=list(left="L",right="R")),
                   factors=list(S=c("left","right"),D=c("easy","hard"),I=c("acc","spd")),
                   constants = c(sd_v.false=1,st0=0), 
                   responses = c("L","R"),
...
# Set parameter mean values
p.vector=c(A=.3,B.acc=1.3,B.spd=.9,t0=.2,
...
## Step 1 - Build model and set prior parameter values
# Build model
model <- model.dmc(p.map     = list(A="1",B="I",t0="1",mean_v=c("D","M"),sd_v="M",st0="1"), 
                   match.map = list(M=list(left="L",right="R")),
                   factors=list(S=c("left","right"),D=c("easy","hard"),I=c("acc","spd")),
                   constants = c(sd_v.false=1,st0=0), 
                   responses = c("L","R"),
